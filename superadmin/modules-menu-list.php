...
<body>
              ...
              <div class="col-12">
                ...
                <div class="card">
                  <div class="card-footer">
                    <div class="row">
                      <div class="col-3"><button class="btn btn-primary btn-block" onclick="showModal('add_menu_modal')"><i class="fas fa-plus"></i> Add new menu</button></div>
                      <div class="col-3">
                              ...
                              for ($i=1; $i <= $num_btn; $i++) {
                                if($i == 1){
                                  ...
                                  <li class="page-item <?php if($cpage == 1){ echo "disabled"; } ?>">
                                    <a class="page-link" href="Javascript:gotoLocalPage('modules-menu-list?uid=<?php echo $uid;?>&page=<?php echo $prevpage;?>')" tabindex="-1"><i class="fas fa-chevron-left"></i></a>
                                  </li>
                                  <?php
                                }
                                ...
                                echo '<li class="page-item '.$mnactive.'"><a class="page-link" href="Javascript:gotoLocalPage(\'modules-menu-list?uid='.$uid.'&page='.$i.'\')">'.$i.' <span class="sr-only">(current)</span></a></li>';

                                if($i == $num_btn){
                                  ?>
                                  <li class="page-item <?php if($i == $cpage){ echo "disabled"; } ?>">
                                    <a class="page-link" href="Javascript:gotoLocalPage('modules-menu-list?uid=<?php echo $uid;?>&page=<?php echo $nextpage;?>')"><i class="fas fa-chevron-right"></i></a>
                                  </li>
                                  <?php
                                }
                              }
                    ...
                    <table class="table mb-0">
                      <thead>
                        <tr class="bg-secondary">
                          ...
                          <th class="text-dark" scope="col">Menu item</th>
                          <th class="text-dark" scope="col">Parent</th>
                          <th class="text-dark" scope="col">Domain</th>
                          <th class="text-dark" scope="col" style="width: 60px;">Visibility</th>
                          <th class="text-dark" scope="col"></th>
                        </tr>
                      </thead>
                      <tbody id="result1-">
                        <?php
                        $strSQL = "SELECT a.ID, a.level, a.menu_name mname, a.url, a.menu_position, a.alt, a.seq, a.parent_id,
                                    a.seq, a.status, a.domain_id, a.mn_lang_id, b.menu_name pname, c.sub_domain
                                   FROM
                                    $tb1 a LEFT JOIN $tb1 b ON a.parent_id = b.ID
                                    LEFT JOIN $tb2 c ON a.domain_id = c.ID
                                   WHERE
                                   a.menu_name LIKE '$search%'
                                   AND a.domain_id = '$domain'
                                   LIMIT $start, $rpp
                                  ";
                        ...
                        if(($domain == '') || ($domain == 'null')){
                          $strSQL = "SELECT a.ID, a.level, a.menu_name mname, a.url, a.menu_position, a.alt, a.seq, a.parent_id,
                                     ...
                                     FROM
                                      ...
                                      LEFT JOIN $tb2 c ON a.domain_id = c.ID
                                     WHERE
                                     (a.domain_id = '' OR a.domain_id = 'null') AND
                                     a.menu_name LIKE '$search%'
                                     LIMIT $start, $rpp
                                    ";
                        }
                        ...
                        if($query){
                          ...
                          while($row = mysqli_fetch_array($query)){
                            $menu_btn = '<button class="btn btn-sm btn-icon" onclick="admin.menu_edit(\''.$row['ID'].'\')"><i class="fas fa-pencil-alt"></i></button>'.
                                        '<button class="btn btn-sm btn-icon" onclick="admin.menu_delete(\''.$row['ID'].'\')"><i class="fas fa-trash"></i></button>';
                                        $visibility = '<label class="custom-switch mt-2 pl-0">'.
                                                        '<input type="checkbox" name="custom-switch-checkbox" class="custom-switch-input" id="txtDie_'.$row['ID'].'" onclick="admin.toggleMenuStage(\''.$row['ID'].'\', \'1\')">'.
                                                        '<span class="custom-switch-indicator"></span>'.
                                                      '</label>';

                                        if($row['status'] == '1'){
                                          $visibility = '<label class="custom-switch mt-2 pl-0">'.
                                                          '<input type="checkbox" name="custom-switch-checkbox" class="custom-switch-input" id="txtDie_'.$row['ID'].'" checked onclick="admin.toggleMenuStage(\''.$row['ID'].'\', \'0\')">'.
                                                          '<span class="custom-switch-indicator"></span>'.
                                                        '</label>';
                                        }
                            ?>
                            <tr>
                              <td><?php echo $c; ?></td>
                              <td><?php echo $row['mname']; ?><div class="fs08">URL : <a href="<?php echo $row['url'];?>" target="_blank"><?php echo $row['url'];?></a></div></td>
                              <td><?php echo $row['pname']; ?></td>
                              <td><?php echo $row['sub_domain']; ?></td>
                              <td><?php echo $visibility; ?></td>
                              <td><?php echo $menu_btn; ?></td>
                            </tr>
                            <?php
                            $c++;
                          }
                        }else{
                          echo '<tr><td colspan="6" scope="row" class="text-center">No items found</td></tr>';
                        }
                        ?>
                      </tbody>
                    </table>
  ...
  <script type="text/javascript">
    ...
    function gotoLocalPage(url){
      // if($('#txtSearch').val() != ''){
      //   window.location = url + '&searchterm=' + $('#txtSearch').val()
      // }
        ...
        <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <select class="form-control" name="txtParent" id="txtParent">
                ...
                <?php
                $tb1 = $db_prefix."menus";
                $strSQL = "SELECT * FROM $tb1 WHERE (domain_id = 'null' OR domain_id = '') AND level = '0'";
                $query = mysqli_query($conn, $strSQL);
                if($query){
                  while($row = mysqli_fetch_array($query)){
                    ?>
        ...
        <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <select class="form-control" name="txtParent2" id="txtParent2">
                ...
                <?php
                $tb1 = $db_prefix."menus";
                $strSQL = "SELECT * FROM $tb1 WHERE (domain_id = 'null' || domain_id = '') AND level = '0'";
                $query = mysqli_query($conn, $strSQL);
                if($query){
                  while($row = mysqli_fetch_array($query)){
                    ?>
